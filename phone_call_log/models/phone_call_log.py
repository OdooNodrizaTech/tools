    ...
    def create(self, values):
        ...
        if return_object.number!=False:
            if '+' not in return_object.number:
                if return_object.user_id.partner_id.country_id.id>0:
            ...
            if phone_is_mobile==True:
                res_partner_ids = self.env['res.partner'].search([('mobile', '=', number_need_check)])
            else:
                res_partner_ids = self.env['res.partner'].search([('phone', '=', number_need_check)])
            #items
            if len(res_partner_ids)>0:
                return_object.partner_id = res_partner_ids[0].id
                #get mail_activity_type
                mail_activity_type_ids = self.env['mail.activity.type'].search([('is_phone_call', '=', True)])
                if len(mail_activity_type_ids)>0:
                    ...
                    if len(ir_model_ids)>0:
                        ...
                        mail_activity_obj = self.env['mail.activity'].sudo(return_object.user_id.id).create(mail_activity_vals)
